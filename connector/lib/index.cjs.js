...
var func = convert_1('isNil', isNil_1, _falseOptions);
...
async function connect(address, port, id) {
...
function connectToBulb(bulb) {
  const address = get$1("address", bulb);

  const port = get$1("port", bulb);

  const id = get$1("id", bulb);
...
var func$2 = convert_1('isString', isString_1, _falseOptions);

func$2.placeholder = placeholder;
var isString$1 = func$2;
...
var func$3 = convert_1('map', map_1);
...
var func$4 = convert_1('join', join_1);

func$4.placeholder = placeholder;
var join$1 = func$4;

async function command(socket, {
  id,
  method,
  params
}) {
  console.log("sending command " + `{"id": ${id}, "method": "${method}", "params": [${params}]}\r\n`);

  const stringifiedParams = join$1(", ", map$1(param => isString$1(param) ? `"${param}"` : param, params));

  const result = await socket.write( // '{"id": 0x0000000007D02FCC,"method": "toggle","params": []}\r\n'
  `{"id": ${id},"method": "${method}", "params": [${stringifiedParams}]}\r\n`, "utf8", (...args) => console.log("callback", ...args));
  return result;
}
async function commandBulb({
  method,
  params
}) {
...
class Bulb {
  constructor(id, address, port = 55443) {
    ...
    this.command = () => function (e) {
      throw e;
    }(new Error("You must call the init function first"));
  }
